...
static esp_err_t cardValid_handler(httpd_req_t *req)
{
    ...
    const char *response = (const char *) req->user_ctx; // copy user ctx "Hello World" into response
    error = httpd_resp_send(req, response, strlen(response)); // server sends response
    if (error != ESP_OK)
    {
        ESP_LOGI(TAG, "Error %d while sending Response", error);
...
\
.button1 {background-color: #00A8FB;} /* Blue */\
\
h1 {\
  text-align: center; /* Aligning the h1 element to the center */\
  color: #0077B6; /* Changing title color to #0077B6 */\
...
\
<button class=\"button button1\" onclick= \"window.location.href='/cardvalid'\">NO CARD</button>\
\
</body>\
</html>\
"
